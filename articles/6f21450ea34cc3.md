---
title: "ChatGPT APIとNext.jsでお手軽チャット作成"
emoji: "🙉"
...
const Chat = ({ content, role }: Message) => {
  const chatStringIndex = useRef(0); // 次に表示する文字のインデックスを保持
  ...
  useEffect(() => {
    ...
    if (chatStringIndex.current < content.length - 1) {
      const appendCharInterval = setInterval(appendChar, 80);
...
詳しいソースコードは以下を見ていただけたらと思います。（私の好きなキャラクターのロリスから名前をとってきています 😳）
